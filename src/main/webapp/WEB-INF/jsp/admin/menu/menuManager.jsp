...
<script>
    layui.extend({
    ...
    }).use(['form', 'table','laydate','jquery','dtree','iconPickerFa'], function () {
         var form = layui.form;
         var $ = layui.jquery;
        ...
        table.on("tool(currentTableFilter)",function (obj) {
            switch (obj.event) {
                case "edit"://编辑按钮
                    openUdateWindow(obj.data);//打开修改窗口
                    break;
                case "delete"://删除按钮
        ...
        function openUdateWindow(data){
            mainIndex = layer.open({
                ...
                success: function () {
                    ...
                    //父级菜单回显
                    dtree.dataInit("menuSelectTree",data.pid);//参数一：下拉菜单树的id属性值 参数二：父节点的ID值
                    dtree.selectVal("menuSelectTree");//参数一：下拉菜单树的id属性值
                    //判断当前选中节点是否是一级菜单，如果是一级菜单，那么该菜单时没有父级菜单，此时应该显示请选择
                    if(data.pid==0){
                        //刷新菜单树
        ...
        function deleteById(data){
            $.get("/admin/menu/checkMenuHasChild",{"id":data.id},function (result){
                    if(result.exist){ //如果该菜单存在子菜单，则进行提示；如果没有子菜单，则提示是否删除
